...
def test_empty_submit_job():
    response = client.post(
        "/job",
        data={"psms": "", "ptm_annotations": "", "background_color": "", "species": ""},
        headers={"Content-Type": "application/json"},
    )
    assert response.status_code == 422
    response = client.post(
        "/job", data={}, headers={"Content-Type": "application/json"}
    )
    assert response.status_code == 422
    response = client.post(
        "/job",
        data={"psms": {}, "ptm_annotations": {}, "background_color": 0, "species": ""},
        headers={"Content-Type": "application/json"},
    )
    assert response.status_code == 422
...
def test_get_invalid_protein_list():
    response = client.post(
        "/protein-list", data={"job": "00000000-0000-0000-0000-000000000000"}
    )
